...
\notes{The *entropy* $H(v_x, v_y)$ for each ball is shown in the top-left of
...
\notes{The coloured dot in the top-right corner of each panel matches the ball's
colour on the main canvas, making it easy to follow individual balls.}

\notes{Use the **Display** dropdown to switch between the 2D joint distribution
...
\notes{Use the **Initialisation** dropdown to choose how the balls start:
...
\notes{For the four directional cases all nine histograms start at the same point,
...
\notes{Notice that this system is ergodic: in the long-run distribution
