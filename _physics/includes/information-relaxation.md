\ifndef{informationRelaxation}
\define{informationRelaxation}
...
Why? Our inspiration is that the second law of thermodynamics tells us that entropy increases. If we want to introduce dynamics in the game, increasing entropy provides an obvious way to do that. Entropy also presents a way of ordering onfigurations that doesn't require us to "see inside" the game. Since $I + H = C$ is constant, if $H$ increases, $I$ must decrease. The system *breaks down correlations* to increase entropy.
