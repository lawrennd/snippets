...
\notes{Week 7's $m$-projection is this matching, written geometrically. Week 8's multi-information $I=\sum h_i-H$ is the extra reduction in joint entropy that $J$ buys. Paths, caliber, and Schrödinger bridges wait until week 8 (second half). The algorithm that finds the MaxEnt *coupling* of two prescribed marginals --- Sinkhorn, or iterative proportional fitting --- waits with them: that is the same exponential family with a different constraint set (the full row and column sums, not three moments). Four states is small enough to write the partition function by hand,
$$
