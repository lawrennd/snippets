...
\subsection{Regime of Quasi-Static Latent Variables: \texorpdfstring{$M$}{M} as Information Topography}
...
\subsubsection{Quasi-Static Approximation: \texorpdfstring{$M$}{M} as Parameters}
...
\subsubsection{Interpretation: \texorpdfstring{$M$}{M} as Informational Topography}

In this regime, the slowly evolving latent variables $M$ do not directly participate in the entropy ascent. Instead, they shape the local curvature structure \textemdash{} i.e., they define an \textbf{information topography} over which the active variables evolve.

* $G_{XX}(\theta_M)$ is the \textbf{local metric}.
* $G_{XM}(\theta_M) \theta_M$ is an \textbf{effective force field} induced by the latent structure.
* The flow $\frac{d\theta_X}{dt}$ is a \textbf{geodesic-like ascent} on this $M$-shaped terrain.

EPI emerges as a \textbf{conditionally local principle}, with $M$ shaping the conditions of variation but not varying itself.
